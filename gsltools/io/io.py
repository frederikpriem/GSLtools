...
dtype_map = [(1, np.uint8),                   # unsigned byte
             ...
             (3, int),                        # 32-bit int (actually platform dependent)
             (4, np.float32),                 # 32-bit float
             (4, float),                      # 32-bit float (actually platform dependent)
...
def envi_to_dtype(envi_dtype):

    lut = dict((k, v) for (k, v) in dtype_map)
    if envi_dtype not in lut.keys():
        raise ValueError('{} not found in ENVI data types'.format(envi_dtype))
    else:
        return lut[envi_dtype]


def dtype_to_envi(dtype):

    lut = dict((v, k) for (k, v) in dtype_map)
    if dtype not in lut.keys():
        raise ValueError('{} not found in allowed built-in or Numpy data types'.format(dtype))
    else:
        return lut[dtype]
